**Code**
- How to optimise reading large datasets in Polars - Claude Sonnet 5 
- How to convert this pandas code into polars ? - Claude Sonnet 5 
- How to use parallel processing to optimise this code - Claude Sonnet 5 
- What is causing the error in this code - Claude Sonnet 5 
- Help format this plot - Claude Sonnet 5 

**Diagrams**
- Help make a tikz figure of this variation of information diagram - Gemini 3.6
- Help make a tikz figure of a bipartite graph - Gemini 3.6


